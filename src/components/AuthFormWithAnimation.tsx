...
export default function AuthFormWithAnimation({ initialMode = 'login' }: AuthFormProps) {
  ...
  // Auto-fill email from URL parameter (from kreator)
  useEffect(() => {
  ...
  const handleRegisterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'phone' && typeof processedValue === 'string') {
      ...
      if (!processedValue.startsWith('+48 ')) {
        // Extract only the numbers
        const numbers = processedValue.replace(/\D/g, '')
        // If it starts with 48, remove it (user typed it)
        const cleanNumbers = numbers.startsWith('48') ? numbers.slice(2) : numbers
        processedValue = '+48 ' + cleanNumbers
      }
      // Limit to +48 and 9 digits
      const numbers = processedValue.slice(4).replace(/\D/g, '').slice(0, 9)
      processedValue = '+48 ' + numbers
    }
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Force redirect with window.location to ensure session cookies are loaded
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-smakowalo-cream to-white flex items-center justify-center p-4" style={{ transform: 'none', writingMode: 'horizontal-tb' }}>
      <div className="w-full max-w-md" style={{ transform: 'none' }}>
                ...
                <form className="space-y-4" onSubmit={handleRegisterSubmit}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={handleRegisterChange}
                      onFocus={(e) => {
                        // Ensure cursor is after "+48 " on focus
                      ...
                      disabled={loading}
                    />
                    <p className="text-xs text-gray-500">Format: +48 xxx xxx xxx (9 cyfr)</p>
                  </div>
