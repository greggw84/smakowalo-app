...
function toImageUrl(imagePath: string): string {
  ...
  // If this is a cached/resized image, extract the original path
  // Example: cache/catalog/product-500x500.jpg -> catalog/product.jpg
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      return productsList.map((p: any) => {
        // IMPORTANT: Use p.image (original) instead of p.thumb (compressed thumbnail)
        const imagePath = p.image || p.thumb || ''
        const imageUrl = toImageUrl(imagePath)

        console.log(`📷 Product ${p.product_id}: ${p.name}`)
        console.log(`   Original image path: ${imagePath}`)
        console.log(`   Full quality URL: ${imageUrl}`)
